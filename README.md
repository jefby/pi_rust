...
| `@earendil-works/pi-coding-agent` | `pi-coding-agent` | `pi` CLI: print mode (`-p`), interactive REPL with streaming render, **session persistence** + `--resume` / `--continue`, **tree sessions** (`/tree`, `/fork`, `/clone`, `--fork`, branch summary), **AGENTS.md / CLAUDE.md loader**, `SYSTEM.md` / `APPEND_SYSTEM.md`, slash commands (`/help /new /reset /model /tools /cost /sessions /resume /session /tree /fork /clone /compact`), interactive permission prompts (`--yolo` to skip), model/provider selection (`--provider`, `--list-models`, `--thinking`). Optional full-screen TUI (`--features tui`) with multi-line input, reasoning display, and light Markdown. `pi sessions list/show/delete/import/export` subcommand (imports/exports upstream `.jsonl` trees). |
...
# Full-screen TUI (optional feature):
cargo build --release --features tui -p pi-coding-agent
./target/release/pi              # auto-starts the TUI on a TTY; --no-tui for the REPL
